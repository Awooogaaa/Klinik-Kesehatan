<x-app-layout>
    <div class="min-h-screen bg-gradient-to-br from-blue-50 via-white to-blue-50 py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-10">
                ...
                <div class="relative z-10 flex flex-col md:flex-row justify-between items-center gap-8">
                    <div>
                        ...
                        <p class="text-gray-600 text-lg max-w-2xl leading-relaxed">
                            Kesehatan Anda adalah prioritas kami. Gunakan dashboard ini untuk mendaftar berobat, mengelola data keluarga, dan memantau riwayat pemeriksaan.
                        </p>
                    </div>
            ...
            {{-- TABEL RIWAYAT & STATUS KUNJUNGAN --}}
                        ...
                        <table class="min-w-full divide-y divide-gray-100">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-8 py-5 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Status Kunjungan</th>
                                    {{-- KOLOM BARU: STATUS PEMBAYARAN --}}
                                    <th class="px-8 py-5 text-center text-xs font-bold text-gray-500 uppercase tracking-wider">Status Pembayaran</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-100">
                                @forelse($riwayat as $kunjungan)
                                    <tr x-data="{ openJadwal: false, openRekam: false }" class="hover:bg-blue-50/30 transition duration-200 group">
                                        ...
                                        {{-- KOLOM 4 (BARU): STATUS PEMBAYARAN --}}
                                        <td class="px-8 py-6 whitespace-nowrap text-center">
                                            @if($kunjungan->status == 'selesai')
                                                ...
                                                @if($pembayaran)
                                                    @if($pembayaran->status_pembayaran == 'lunas')
                                                        <div class="flex flex-col items-center gap-2">
                                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-green-100 text-green-800 border border-green-200">
                                                                LUNAS ({{ ucfirst($pembayaran->metode_pembayaran ?? 'Online') }})
                                                            </span>
                                                    ...
                                                    @else
                                                        <div class="flex flex-col items-center gap-2">
                                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-yellow-100 text-yellow-800 border border-yellow-200 animate-pulse">
                                                                MENUNGGU
                                                            </span>
                                                            <a href="{{ route('pembayarans.show', $pembayaran->id) }}" class="inline-flex items-center justify-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-full shadow-sm text-white bg-yellow-600 hover:bg-yellow-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yellow-500">
                                                                Bayar Sekarang
                                                            </a>
                                                        </div>
                                                    @endif
                                                                            ...
                                                                            <ul class="space-y-2">
                                                                                @foreach ($kunjungan->rekamMedis->obats as $obat)
                                                                                    <li class="flex justify-between items-center bg-gray-50 px-3.5 py-2.5 rounded-xl border border-gray-100 text-sm hover:border-purple-200 transition duration-200 group">
                                                                                        ...
                                                                                        <span class="text-[10px] font-bold text-purple-600 bg-white px-2 py-1 rounded-lg border border-purple-100 shadow-sm">{{ $obat->pivot->dosis }}</span>
                                                                                    </li>
                                                                                @endforeach
                                                                            </ul>
                                                                        @else
